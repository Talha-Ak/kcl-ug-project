...
object ClosureParser {
    ...
    case class Closure(name: String, args: Seq[String], body: Exp) extends Exp
    ...
    def IdParser[$: P] =
        P(!StringIn("if", "then", "else", "write", "def", "indef") ~ CharIn("A-Za-z") ~~ CharsWhileIn("A-Za-z0-9_", 0)).!

    // Expressions
    //===============
    def Exp[$: P]: P[Exp] = P(
        ("if" ~ BExp ~ "then" ~ Exp ~ "else" ~ Exp).map(If) |
        ("indef" ~ IdParser ~ "(" ~ IdParser.rep(0, ",") ~ ")" ~ "=" ~/ Exp).map(Closure) |
