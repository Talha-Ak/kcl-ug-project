...
class NewCPS(counter: Counter) {
    ...
    def CPS(e: Exp, ty: TypeEnv = Map())(k: (KVal, TypeEnv) => KAnf) : KAnf = e match {
        ...
        case Const(i, t, v) =>
            ...
            CPS(v, updated_ty)((y, t1) => KConst(i, y, k(KVar(i, t), t1)))
        case Func(name, args, ret, body) =>
            val updated_ty = ty ++ args.map{case (x, t) => (x, t)} + (name -> FnType(args.map(_._2).toList, ret))
            KFun(name, args, ret, CPS(body, updated_ty)((y, _) => KReturn(y)), k(KVar(name, FnType(args.map(_._2).toList, ret)), ty + (name -> FnType(args.map(_._2).toList, ret))))
        case StructDef(name, items) =>
            ...
            KStructDef(Struct(name, items), k(KVar(name, UserType(name)), updated_ty))
